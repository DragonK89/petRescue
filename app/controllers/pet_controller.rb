class PetController < ApplicationController
	...
	def register
		if !params[:file]
			return render :json => {status: 0,message:"pls put pet image"}
		end
		if params[:clinic_id]
	...
	def register_clinic
		if @user_id
			...
			render :json => {status: 1,message:"registed pet id "+pet.id+" to clinic id "+clinic.id+" success"}
		else
	...
	def pending_user
		...
		if !pet
			return render :json => {status: 0,message:"cant find this pet"}
		end
	...
	def register_user
		...
		if confirm_code[0]["user_id"]
			...
			if !user
				return render :json => {status:0,message:"cant find user"}
			end
		end
		pet.update(user_id:user.id,confirm_code: nil,health_status: 2)
		render :json => {status: 1,message:"registed pet id "+pet.id.to_s+" to user id "+user.id.to_s+" success"}
	end
